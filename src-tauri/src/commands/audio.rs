...
use onda_audio::{AudioCommand, EqBand, MAX_GAIN_DB, PlaybackState, BAND_COUNT};
...
pub fn set_volume(state: State<'_, AppState>, volume: f32) -> Result<(), OndaError> {
    if !(0.0..=1.0).contains(&volume) {
        return Err(OndaError::InvalidArgument("volume must be within 0.0..=1.0".into()));
    }
...
pub fn set_eq_gain(state: State<'_, AppState>, band: u8, gain_db: f32) -> Result<(), OndaError> {
    if band as usize >= BAND_COUNT {
        return Err(OndaError::InvalidArgument(format!("band must be < {BAND_COUNT}")));
    }
    if !gain_db.is_finite() || gain_db.abs() > MAX_GAIN_DB {
        return Err(OndaError::InvalidArgument(format!("gain_db must be within ±{MAX_GAIN_DB}")));
    }
