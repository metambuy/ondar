...
export_bindings_<type>` that writes the `.ts` file. So the 231 tests `cargo test --workspace`
reports break down as **212 hand-written + 19 ts-rs-generated** (audio 127, shell 40, stations 64):
...
Counting `#[test]` attributes in source gives 212 and will not reconcile with the runner's 231
...
Every "tests" figure in this project is written as the two numbers, `231 + 15`, never their sum:
...
pnpm test                    # vitest under jsdom, `src/**/*.test.tsx` (M3b 1b): the renderer's own
                              ...
                              # the Rust count — "231 + 15", never "246" — and CI runs it as its own step
...
cargo test --workspace       # 231 tests: 127 in the ondar_audio binary, 64 in ondar_stations and
