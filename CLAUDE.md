...
export_bindings_<type>` that writes the `.ts` file. So the 219 tests `cargo test --workspace`
reports break down as **200 hand-written + 19 ts-rs-generated** (audio 115, shell 40, stations 64):
...
Counting `#[test]` attributes in source gives 200 and will not reconcile with the runner's 219
...
Every "tests" figure in this project is written as the two numbers, `219 + 15`, never their sum:
...
pnpm test                    # vitest under jsdom, `src/**/*.test.tsx` (M3b 1b): the renderer's own
                              ...
                              # the Rust count — "219 + 15", never "234" — and CI runs it as its own step
...
cargo test --workspace       # 219 tests: 115 in the ondar_audio binary, 64 in ondar_stations and
