...
export_bindings_<type>` that writes the `.ts` file. So the 159 tests `cargo test --workspace`
reports break down as **140 hand-written + 19 ts-rs-generated** (audio 63, shell 40, stations 56):
...
Counting `#[test]` attributes in source gives 140 and will not reconcile with the runner's 159
...
cargo test --workspace       # 159 tests: 63 in the ondar_audio binary, 56 in ondar_stations and
