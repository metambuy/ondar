...
export_bindings_<type>` that writes the `.ts` file. So the 233 tests `cargo test --workspace`
reports break down as **214 hand-written + 19 ts-rs-generated** (audio 129, shell 40, stations 64):
...
Counting `#[test]` attributes in source gives 214 and will not reconcile with the runner's 233
...
Every "tests" figure in this project is written as the two numbers, `233 + 15`, never their sum:
...
pnpm test                    # vitest under jsdom, `src/**/*.test.tsx` (M3b 1b): the renderer's own
                              ...
                              # the Rust count — "233 + 15", never "248" — and CI runs it as its own step
...
cargo test --workspace       # 233 tests: 129 in the ondar_audio binary, 64 in ondar_stations and
