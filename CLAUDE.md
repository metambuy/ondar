...
export_bindings_<type>` that writes the `.ts` file. So the 235 tests `cargo test --workspace`
reports break down as **216 hand-written + 19 ts-rs-generated** (audio 131, shell 40, stations 64):
...
Counting `#[test]` attributes in source gives 216 and will not reconcile with the runner's 235
...
Every "tests" figure in this project is written as the two numbers, `235 + 15`, never their sum:
