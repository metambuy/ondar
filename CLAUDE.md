...
export_bindings_<type>` that writes the `.ts` file. So the 232 tests `cargo test --workspace`
reports break down as **213 hand-written + 19 ts-rs-generated** (audio 128, shell 40, stations 64):
...
Counting `#[test]` attributes in source gives 213 and will not reconcile with the runner's 232
...
Every "tests" figure in this project is written as the two numbers, `232 + 15`, never their sum:
...
pnpm test                    # vitest under jsdom, `src/**/*.test.tsx` (M3b 1b): the renderer's own
                              ...
                              # the Rust count — "232 + 15", never "247" — and CI runs it as its own step
...
cargo test --workspace       # 232 tests: 128 in the ondar_audio binary, 64 in ondar_stations and
