...
export_bindings_<type>` that writes the `.ts` file. So the 227 tests `cargo test --workspace`
reports break down as **208 hand-written + 19 ts-rs-generated** (audio 123, shell 40, stations 64):
...
Counting `#[test]` attributes in source gives 208 and will not reconcile with the runner's 227
...
Every "tests" figure in this project is written as the two numbers, `227 + 15`, never their sum:
...
pnpm test                    # vitest under jsdom, `src/**/*.test.tsx` (M3b 1b): the renderer's own
                              ...
                              # the Rust count — "227 + 15", never "242" — and CI runs it as its own step
...
cargo test --workspace       # 227 tests: 123 in the ondar_audio binary, 64 in ondar_stations and
