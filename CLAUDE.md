...
| M3 | Station API + SQLite cache + country/station UI | **in progress** — Step 0 live-data census done 2026-09-21 (`_handover/m3-step0-report.md`; one API server, silent 1000-row default, 20.7 % geo, HLS 3.8 %, Shoutcast v1 0/148); M3a (crate, cache, commands) on branch `m3a`, **acceptance run 2026-09-22** (10 items; two fixes `3ab7ec2` retry policy by cause, `4d83918` failed-refresh event; the re-request on show/reconnect carried to M3b) |
...
└── src-tauri/
    ...
    └── crates/ondar-stations/    the station directory (M3a). No Tauri dependency.
        ...
        │                         the three-rule truncation guard
...
export_bindings_<type>` that writes the `.ts` file. So the 149 tests `cargo test --workspace`
reports break down as **130 hand-written + 19 ts-rs-generated** (audio 58, shell 40, stations 51):
...
Counting `#[test]` attributes in source gives 130 and will not reconcile with the runner's 149
...
cargo test --workspace       # 149 tests: 58 in the ondar_audio binary, 51 in ondar_stations and
...
with nothing cached, a truncated list, a cache failure), or `invalid_argument` for a bad country
