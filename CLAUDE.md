...
export_bindings_<type>` that writes the `.ts` file. So the 222 tests `cargo test --workspace`
reports break down as **203 hand-written + 19 ts-rs-generated** (audio 118, shell 40, stations 64):
...
Counting `#[test]` attributes in source gives 203 and will not reconcile with the runner's 222
...
Every "tests" figure in this project is written as the two numbers, `222 + 15`, never their sum:
...
pnpm test                    # vitest under jsdom, `src/**/*.test.tsx` (M3b 1b): the renderer's own
                              ...
                              # the Rust count — "222 + 15", never "237" — and CI runs it as its own step
...
cargo test --workspace       # 222 tests: 118 in the ondar_audio binary, 64 in ondar_stations and
